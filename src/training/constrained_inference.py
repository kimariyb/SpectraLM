"""Formula-constrained candidate generation and model-based ranking."""
...
from src.data.dataset import load_sample_images
from src.data.modalities import input_mode_uses_images, validate_input_configuration
...
def constrain_and_rank_sample(
    ...
    *,
    ...
    """Apply hard constraints and rank one generated candidate set."""
    formula = sample.get("molecular_formula") or sample.get("formula")
    filtered = filter_generated_candidates(
        generated_candidates,
        molecular_formula=str(formula) if formula else None,
    )
    ...
    return {
        ...
        "domain_valid_candidate_count": len(
            filtered.domain_valid_candidates
        ),
        "formula_valid_candidate_count": len(
            filtered.formula_valid_candidates
        ),
...
def main(config: dict[str, Any]) -> None:
    ...
    # GPU-heavy imports remain lazy so pure constraint logic is CPU-testable.
    import torch
    ...
    input_mode = validate_input_configuration(
        ...
        include_rule_context=bool(config.get("rule_context_enabled", False)),
    )
    ...
    template = select_structure_prompt(
        int(config.get("prompt_template_index", 0)),
        ...
        for index, sample in tqdm(iterator, desc="Constrained infer"):
            ...
            prompt = build_structure_prompt(
                ...
                include_rule_context=bool(
                    config.get("rule_context_enabled", False)
                ),
