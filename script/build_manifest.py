"""Build QC and scaffold-disjoint split manifests for SpectraLM datasets."""
...
from src.io import load_pickle_list, write_json, write_rows_csv
...
def assign_scaffold_splits(
    ...
    *,
    ...
    """Assign train/val/test splits with scaffold-level exclusivity."""
    ...
    train_target = int(n_pass * train_ratio)
    val_target = int(n_pass * val_ratio)
    ...
    decorated_scaffolds = [(rng.random(), scaffold) for scaffold in by_scaffold]
    scaffolds = [
        scaffold
...
def manifest_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    """Summarize manifest rows for reporting."""
...
def build_manifest(
    ...
    *,
    ...
    """Build manifest rows and summary from normalized samples."""
...
def write_split_files(rows: list[dict[str, Any]], out_dir: Path) -> None:
    """Write one id list per benchmark split."""
...
def main() -> None:
    ...
    parser.add_argument("dataset", help="Normalized SpectraLM pickle dataset.")
    parser.add_argument(
        "--out-dir",
        default="dataset/manifests",
    ...
    samples = load_pickle_list(args.dataset)
    ...
    write_rows_csv(out_dir / "manifest.csv", rows, MANIFEST_FIELDS)
    write_json(out_dir / "manifest_summary.json", summary)
    write_split_files(rows, out_dir)
